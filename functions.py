...
def plot_model_statistics_simple(
        path2figures:str,
        obs_source:str,
        station_meta:dict,
        quantiles:np.ndarray,
        ...
        percentile_temps:dict,
        probabilities: dict,
        qr_sm_dict:dict[str,pd.DataFrame])->None:
    ...
    Args:
        ...
        station_meta: dict
            Station metadata
        quantiles: np.ndarray
            A list of quantiles (e.g. 0.01...0.99)
        doy_index: int
            day_of_year index
        n_days: int
            Number of days in the period
        index: int
            The index of target_observations in the 'T_range' array
        target_value: float
            Target observation
        pwarm: bool
            Probabilty for warmer (True)/colder (False) temperatures
        ...
        percentile_temps: dict
            Contains temperatures corresponding to a given percentile in:
                 ...
                 - Future climate (Optional)
        qr_sm_dict: dict
            Contains single-model quantiles for the given period and day of year. 
    Returns:
        None: The plot is saved as a PNG-figure.
    """
    ...
    def get_dI_single_models(CDF_sm_preind:np.ndarray, CDF_sm_target:np.ndarray,T_range:np.ndarray, prob:float)->np.ndarray:

        T_preind = np.empty(CDF_sm_preind.shape[-1])
        T_target = np.empty(CDF_sm_preind.shape[-1])

        for i in range(CDF_sm_preind.shape[-1]):
            T_preind[i] = temp_at_prob(CDF_sm_preind[:,i], T_range, prob)
        
        for i in range(CDF_sm_preind.shape[-1]):
            T_target[i] = temp_at_prob(CDF_sm_target[:,i], T_range, prob)

        return T_target - T_preind
    ...
    # Get bootstrap results to arrays
    qr_sm_preind = qr_sm_dict["preind"]
    qr_sm_target = qr_sm_dict["target"]

    # Get single model CDFs
    CDF_sm_preind = get_CDF_single_models(qr_sm_preind, quantiles, T_range).T
    CDF_sm_target = get_CDF_single_models(qr_sm_target, quantiles, T_range).T
    ...
    # Compute the probability ratios for each model (probability of target period exceeding pre-industrial)
    if pwarm:
        PRratios = np.transpose((1-CDF_sm_target[index,:]) / (1-CDF_sm_preind[index,:]))
    else:
        PRratios = np.transpose(CDF_sm_preind[index,:] / CDF_sm_target[index,:])
    ...
    # Adjust axes
    ax.yaxis.tick_right()  # Move the y-axis ticks to the right
    ax.set_xlim(1, np.max(filtered_data)+5)
    ax.set_xlabel("Probability ratio")
    ax.set_xscale('log')
    ax.invert_yaxis()
    ...
    # Calculate single model intensity changes
    deltaI = get_dI_single_models(CDF_sm_preind, CDF_sm_target, T_range, prob)
    deltaI_data = [[v] for v in deltaI]
    ...
    # Add a box plot for the combined intensity change data for the MMM
    b3 = ax.boxplot([np.concatenate(deltaI_data)],
                    positions=[n_models],
                    tick_labels=['MMM'],
                    vert=False,
                    showfliers=False,  
